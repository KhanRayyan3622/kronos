...
- Restructured the module logic into a `\TLV input_debouncer(/_top)` macro, following
  the task's example. `/_top` is declared per convention but unused (nothing in this
  design's logic needs to reference the instantiation scope explicitly).
- The module's own `\TLV` region now only connects the input (`$gpio_in[N-1:0] =
  *gpio_in;`) and instantiates the macro (`m5+input_debouncer(/top)`).
- Deviation from the task's simple example: the *output* connection
  (`*read[gpio] = $read;`) stays *inside* the macro (inside the `/gpio` scope), rather
  than being pulled out into the module's own `\TLV` region after the macro
  instantiation. This is because the connection is inherently per-replica (it needs
  the `/gpio` scope's index to know which output bit to drive) — the task's own
  example has no replication scope, so it doesn't address this case. Keeping it inside
  the macro (as the last statement of the `/gpio` scope) is the natural fit, and
  `fev.sh` confirms it works correctly.
- `fev.sh` passed on the first attempt.

## Consolidate the SV-TLV Interface notes

- Added `$gpio_in[N-1:0] = *gpio_in;` ("Connect Verilog inputs:") at the top of the
  `\TLV` region, and replaced the one remaining raw Verilog-signal use in logic
  (`*gpio_in[gpio]` in `$sync`'s expression) with the pipesignal `$gpio_in` (referenced
  as `|default$gpio_in[gpio]` from within `/gpio`, since it's assigned at the ancestor
  `|default@0` scope — same ancestor-qualification rule already seen for `$tick`).
  `rename_sigs.py -t gpio_in` confirmed naming compliance before use.
- The output connection (`*read[gpio] = $read;`, "Connect Verilog outputs:") already
  existed from the "For Loop Assignments" task and needed no logic changes — only a
  clarifying comment was added. Note this connection lives inside the `/gpio` scope
  rather than literally "at the end of the last `\TLV` region" as the task's simple
  (non-replicated) example shows; that's unavoidable here since each replica must
  connect its own output bit. No unmerged output-cutpoint FEV issues arose from this
  (unlike the task's warned-about scenario) — verified by a clean `fev.sh` pass with
  no `[collect]`/`[partition]` changes beyond the pre-existing `group *`.
- `rstz` remains a plain Verilog signal used only within the (necessarily un-converted)
  `\SV_plus` async-reset-synchronizer block — not part of `\TLV` logic, so it's outside
  this task's "eliminate Verilog signals from logic expressions" scope. `clk` remains
  implicit per instructions (no `$clk` created).
- After this task, no Verilog signals (`*`-prefixed or otherwise) appear anywhere in
  `\TLV` logic expressions — only in the two connection statements. `fev.sh` passed.

## Review Commenting and Structure notes

- Compared `wip.tlv` against `prepared.sv`: overall ordering (reset synchronizer,
  timer, tick, then per-GPIO sync/raw_val/poll/line/read logic) already matches the
  original. Restored one comment that had been dropped during conversion (`// sync
  inputs`, above the `$sync` assignment in the `/gpio` scope). No other comments were
  missing or reordered; no further changes needed. `fev.sh` passed.

## Convert Remaining Signals to Pipesignals notes

- Converted `resetn`/`resetn_meta` (the async-reset synchronizer state) to pipesignals
  using the `$$`/`$` prefix mechanism, while keeping the `always_ff`/`if`/`else`
  Verilog structure and the `posedge clk or negedge rstz` sensitivity list completely
  unchanged (only the signal names gained `$`/`$$` prefixes). This succeeded where the
  earlier ternary-expression attempt (in the "If/Else and Case to Ternary" task) had
  failed — the `$$`/`$` mechanism doesn't require restructuring the assignment into a
  TLV expression, so yosys's `proc` pass still sees the canonical async-reset `if`
  form it needs. `$timer`'s expression now references `$resetn` directly (dropping the
  earlier `*resetn` plain-Verilog-signal prefix).
- Added `fev.eqy` matches for `resetn`/`resetn_meta`.
- **Blocking issue and resolution — incremental FEV timeout**: `fev.sh` failed 4
  consecutive times with `EQY Failure Analysis: ... All partitions passed!` alongside
  `Incremental FEV failed` — i.e., every individual partition *proved*, but the
  ~160-partition run (16 `gpio` instances x up to 5 signals, plus `resetn`,
  `resetn_meta`, `timer`, `tick`, etc., each auto-partitioned individually) exceeded
  `fev.sh`'s hardcoded 120s `eqy` timeout on this environment. This is a genuine
  capacity limit (partition-startup overhead x count), not a logic problem, and it
  will likely recur and worsen for any design with N=16+ replicated instances once
  most/all of their per-instance signals become individual pipesignal cutpoints.
  Per `fev.eqy`'s own template ("If you want to force grouping by register name
  patterns, you can uncomment: group *"), enabled `group *` in `fev.eqy`'s
  `[collect *]` section. This merges the many small per-signal/per-instance partitions
  into far fewer, larger ones, cutting total EQY startup overhead enough to finish
  within the timeout — incremental FEV then passed.
  - This exposed a **second, pre-existing latent bug**: `resetn`/`resetn_meta`
    (introduced during the Reset and Clock task, when `prepared.sv` was updated) were
    never added to `fev_full*.eqy`'s match sections, since at the time they were named
    identically in gold and gate (no match needed). Once they became pipesignals with
    SandPiper-generated gate names, full FEV failed for the `timer` partition with
    `resetn`/`resetn_meta` unmatched-internal on both sides. Fixed by adding
    `gold-match resetn ...` / `gold-match resetn_meta ...` directly to all three
    `fev_full*.eqy` files, per `instructions/full_fev_failed.md`.
  - **Recommendation for the process/tooling**: `fev.sh`'s incremental-FEV timeout
    could be made configurable, or `[collect] group *` could be enabled by default
    once a design's replicated-instance count crosses some threshold, to avoid this
    becoming a recurring stall for wider/larger arrays.
- `fev.sh` passed (both incremental with `group *` and all full FEV runs).
