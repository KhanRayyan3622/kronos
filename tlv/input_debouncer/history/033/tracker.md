...
- `fev.sh` passed after these fixes. Remaining signals (`raw_val`, `line`, `poll`,
  `read`) still to convert; `read` (a module output) is expected to be the hardest,
  given known EQY output-cutpoint complications flagged for the later "Consolidate the
  SV-TLV Interface" task.
