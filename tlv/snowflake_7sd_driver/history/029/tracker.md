...
## Consolidate the SV-TLV Interface
- Input pipesignals: `$rstz`, `$en` connect directly. `a`/`b` are single-letter names that violate pipesignal naming rule 3 (must begin with two letters), so per the doubling convention (`a` -> `aa` per instructions example) they became `$aa`/`$bb`.
- `sel` and `disp` (module outputs) are now fully pipesignals (`<<1$sel`, `<<1$disp`), wired out via `*sel = $sel;` / `*disp = $disp;`. No `\SV_plus` remains in the file.
- Explicit `fev.eqy` match lines for `sel`/`disp` (`gold-match sel |default<>0$sel`) caused an EQY partition conflict, since the wip netlist retains the literal name `sel`/`disp` for that state bit after `clean` (so it was already being matched automatically by name). Removed the explicit lines; automatic by-name matching sufficed.
